...
class BigQueryService {
  constructor() {
    ...
    this.tableId = config.bigQuery.tableId;
  }
  ...
  async getTableData() {
    try {
      console.log('BigQuery Config:', {
        ...
        hasCredentials: !!process.env.BIGQUERY_CLIENT_EMAIL && !!process.env.BIGQUERY_PRIVATE_KEY
      });

      const query = `SELECT * FROM \`${config.bigQuery.projectId}.${this.datasetId}.${this.tableId}\` LIMIT 1000`;
