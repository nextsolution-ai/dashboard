...
class BigQueryService {
  ...
  async getAuthToken() {
    try {
      console.log('Getting auth token from:', config.bigQuery.authTokenUrl);
  ...
  async getTableData() {
    try {
      ...
      console.log('Fetching BigQuery data from:', url);
      const response = await axios.get(url, { headers });
      
      return this.processDataForDashboard(response.data.rows || []);
    } catch (error) {
      console.error('Failed to fetch data from BigQuery:', error);
      throw error;
    }
  }
